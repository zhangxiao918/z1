...
public class ArrayUtils extends org.apache.commons.lang.ArrayUtils {
    ...
    public static byte[][] clone(byte[][] args) {
        if (args == null) {
            return null;
        }
        if (args.length == 0) {
        ...
        byte[][] retVal = new byte[args.length][args[0].length];

        for (int i = 0; i < retVal.length; i++) {
            System.arraycopy(args[i], 0, retVal[i], 0, args[i].length);
        }
    ...
    public static long[] random(long[] args) {
        ...
        for (int i = 0; i < ret.length; i++) {
            Random r = new Random();
            int ceil = args.length;
            int randomN = (int) (ceil * r.nextDouble());
            ret[i] = args[randomN];
            args = ArrayUtils.removeElement(args, ret[i]);
        }

        return ret;
    }

    public static boolean exist(final byte[] args, final byte[] valuesToFind) {
        if (args == null || args.length == 0) {
            return false;
        }
        if (valuesToFind == null || valuesToFind.length == 0) {
            return false;
        }

        boolean ret = false;
        for (byte valueToFind : valuesToFind) {
            if (count(args, valueToFind) > 0) {
                ret = true;
                break;
            }
        }
    ...
    public static int count(final byte[] args, final byte valueToFind) {
        if (args == null || args.length == 0) {
            return 0;
        }
    ...
    public static int count(final short[] args, final short valueToFind) {
        if (args == null || args.length == 0) {
            return 0;
        }
    ...
    public static String toString(byte[][] args) {
        ...
        StringBuffer retVal = new StringBuffer(toString(args[0])).append("\n");
        for (int i = 1; i < args.length; i++) {
            retVal.append(toString(args[i])).append("\n");
        }
        return retVal.toString();
    }
