...
public class MainActivity extends BaseActivity {
     ...
     */
    void initNetwork() {
        go.request(MobileGo.HZGJJ_LOGIN_URL);
    ...
    public void submit(View view) {
        ...
        handler2.post(new Runnable() {
            public void run() {
                if (r.length() > 2) {
                    Intent i = new Intent(MainActivity.this, ShowContentActivity.class);
    ...
    void loadImage2(final String site, final int id) {
        new Thread(new Runnable() {
            ...
            public void run() {
                synchronized (this) {
                    if (null != go) {
                        byte[] bit = go.requestCheckCode(MobileGo.HZGJJ_CHECKCODE_URL);
